...
import elevator.javaAStar;

import au.edu.rmit.elevator.common.*;

public class AStarServerEnvironment extends Environment implements ElevatorService {

	private Logger logger = Logger.getLogger("Elevator Simulator in Jason."
			+ AStarServerEnvironment.class.getName());
	...
	public AStarServerEnvironment() throws RemoteException {
		// logger.setLevel(Level.WARNING);
	}

	private static AStarServerEnvironment instance;
	 ...
	 */
	static public AStarServerEnvironment getInstance() {
		return instance;
	}
		...
		if (running)
			try {
				if (act.equals("set_destination")) {
					...
					StepHandler.trigger();
				} else if (act.equals("remove_destinations")) {
					removePercept(agName,
							ASSyntax.createLiteral("destinations", action.getTerm(0), action.getTerm(1)));
				} else if (act.equals("print_destinations")) {
					List<Literal> percepts=instance.getPercepts(agName);
					if (percepts!=null) logger.info(percepts.toString());
					if (percepts!=null && !percepts.isEmpty()){
						for (Literal pcpt:percepts){
							ListTerm listTerm=pcpt.getAsListOfTerms();
							if (listTerm.get(0).toString().equals("destinations"))
							//if (pcpt.getFunctor().equals("destinations"))
								logger.info(listTerm.toString());
						}
					}
				}
	...
	public boolean arrive(String car, int floor) throws RemoteException {
		...
		//		+ (dir ? "up" : "down"));
		removePerceptsByUnif(ASSyntax.createLiteral("request",
				ASSyntax.createNumber(floor),
				ASSyntax.createVar()));
				//ASSyntax.createAtom(dir ? "up" : "down")));
		removePercept(car,
				ASSyntax.createLiteral("request", ASSyntax.createNumber(floor)));
		removePercept(ASSyntax.createLiteral("inside_request", ASSyntax.createString(car),ASSyntax.createNumber(floor)));
		removePerceptsByUnif(car,
				ASSyntax.createLiteral("at", ASSyntax.createVar()));
	...
	public Integer getDestination(String car){
		return destinations.get(car);
	}

	public void requestFloor(String car, int floor) throws RemoteException {
		addPercept(car,
				ASSyntax.createLiteral("request", ASSyntax.createNumber(floor)));
		addPercept(ASSyntax.createLiteral("inside_request", ASSyntax.createString(car),ASSyntax.createNumber(floor)));
	}
