...
-at(X) : direction(Direction) & destinations(X,N)
...
+at(X):not (destination(_) | checkpoint(_,_) | destinations(_,_))
<-
.print("I'm stopping now");
stop.

+at(X) : destinations(X,N)
<-
.print("I'm now at ",X);
!reset_extremes;
!check_direction;
...
+!determine_next_destination:next(N) & destinations(N,Step) & not (request(N) | request(N,D))
<-remove_destinations(N,Step);
!determine_next_destination.
...
+?direction_at(Floor,Direction):next(Floor) & destinations(N,A) & (not (destinations(M,B) & (B<A))) & (Floor>N)
<-Direction=down.
...
+!check_direction: at(X) & next(Y) & Y<X
<-
.print("Will go down now");
-direction(_);
+direction(down).
